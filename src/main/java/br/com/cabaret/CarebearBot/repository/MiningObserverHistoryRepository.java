...
public interface MiningObserverHistoryRepository  extends JpaRepository<MiningObserverHistory, Long> {
	...
	@Query(value ="select cm.character_name characterName, moh.type_description typeName, sum(quantity) qtMining, format((sum(quantity) * p.taxa_mining)/100,0) taxaCorp " + 
			...
			"union ALL                                                                                                                      " + 
			"select g.grupo_name characterName, moh.type_description typeName, sum(quantity) qtMining, format((sum(quantity) * p.taxa_mining)/100,0) taxaCorp               " + 
			"  from mining_observer_history moh                                                                                             " + 
